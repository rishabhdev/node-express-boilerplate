...
const getOptionSymbols = (assetPrice) => {
  ...
  const currentExpiryStrikesCE = strikes.map(strike => ({ type: 'CE', symbol: `NIFTY${yCurrentExpiry}${mCurrentExpiry}${dCurrentExpiry}${strike}CE`, expiry: expiryDates[currentExpiryIndex] }));
  const currentExpiryStrikesPE = strikes.map(strike => ({ type: 'PE', symbol: `NIFTY${yCurrentExpiry}${mCurrentExpiry}${dCurrentExpiry}${strike}PE`, expiry: expiryDates[currentExpiryIndex] }));
  const nextExpiryStrikesCE = strikes.map(strike => ({ type: 'CE', symbol: `NIFTY${yNextExpiry}${mNextExpiry}${dNextExpiry}${strike}CE`, expiry: expiryDates[nextExpiryIndex] }));
  const nextExpiryStrikesPE = strikes.map(strike => ({ type: 'PE', symbol: `NIFTY${yNextExpiry}${mNextExpiry}${dNextExpiry}${strike}PE`, expiry: expiryDates[nextExpiryIndex] }));
