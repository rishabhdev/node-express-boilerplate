 ...
 */
 ...
 export function getIv(s, k, t, o, callPut) {
  ...
  while ((Math.abs(pLow - o) >= 0.1) && (i < MAX_ITERATIONS)) {
    i++;
    // console.log({ i, lowerBound, upperBound });
    pLow = blackScholes(s, k, t, lowerBound, r, callPut);
    pHigh = blackScholes(s, k, t, upperBound, r, callPut);
 ...
 export const yearsFromExpiry = (expiry) => {
  ...
  const expiryMmDdYyyy = moment(expiry).format('MM/DD/YYYY')
  let remaingTimeInCurrentDay;
  if (hours > 15) {
    remaingTimeInCurrentDay = 0;
  } else if (hours < 9) {
